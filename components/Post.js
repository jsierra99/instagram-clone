...
const Post = ({ id, username, userImg, img, caption }) => 
{
    ...
    useEffect(
        () =>
            setHasLiked(
                likes.findIndex((x) => x.id === session?.user?.uid) !== -1
            ),
        [likes]
    );
                ...
                <div className='ml-10 h-20 overflow-y-scroll scrollbar-thumb-black scrollbar-thin'>
                    {comments.map(x => (
                        <div key={x.id} className='flex items-center space-x-2 mb-3'>
                            <img className='h-7 rounded-full' src={x.data().userImage} alt="" />
                            <p className='text-sm flex-1'><span className='font-bold'>{x.data().username}</span> {x.data().comment}</p>
